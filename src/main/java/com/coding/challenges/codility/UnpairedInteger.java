...
// element of the array can be paired with another element that has the same value, except for one element that is
// left unpaired.
//
...
public class UnpairedInteger {
    ...
    public static void main(String[] args) {
        ...
        int solution = unpairedInteger.solution(9, 3, 9, 3, 9, 7, 9);
    ...
    public int solution(int... A) {
        ...
        Map<Integer, Long> pairedMap = Arrays.stream(A)
                .boxed()
                .collect(Collectors.groupingBy(i -> i, Collectors.counting()));

        for (Map.Entry<Integer, Long> entry : pairedMap.entrySet())
